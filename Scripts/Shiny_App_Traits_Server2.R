...
server2 <- function(input, output, session) {
  ...
  #################### Update Related Caculation #################### 
  ## Phyloposit
  phyloposi_sp = reactive({
    
    plot_dat = plot_react()
    
    tree_species = PruneTree(input$checked_species)
    dist = cophenetic.phylo(tree_species)
    phyloposi = isoMDS(dist) %>% as.data.frame()
    phyloposi_sp = phyloposi %>% mutate(Species = row.names(phyloposi)) %>% 
      mutate(phy1 = round(scale(points.1),digits = 1)) %>% 
      mutate(phy2 = round(scale(points.2), digits = 1))
    phyloposi_sp
  })
  
  phyloposi_species = phyloposi_sp()
   ...
   plot_phyloposit_fam = reactive({
    ...
    plot_phyloposit_fam = merge(filter(dat, Species %in% input$checked_species), phyloposi_species) %>% 
      group_by(Family) %>% 
  ...
  ## ModoelSelection
  AllMatrix_G_scal = reactive({
    
    AllMatrix = merge(plot_dat, phyloposi_species)
    
    AllMatrix_G = filter(merge(AllMatrix, Germination), Species %in% input$checked_species)
    
    AllMatrix_G_scal = AllMatrix_G %>% 
  ...
  AllMatrix_G_scaled = AllMatrix_G_scal()
